...
The real prediction code looks like [this](https://github.com/ovainola/SnakeTamer/blob/master/dqn.py#L54) and the decay variable update can be seen from [here](https://github.com/ovainola/SnakeTamer/blob/master/dqn.py#L70).

In the supervised learning we'll have the input and expected output. In the unsupervised learning we only have the input and we need reward to score our prediction. Applying the rewards to loss function is done as:
...
Next the rewards. I figured, that if snake collides with itself, I'd give -10. If snake moves closer to the apple, I'll give 1 and if further -1. Lastly if snake eats the apple, I'll reward it with 10. I picked these values, since I felt like it. No magic there :).

## Game and results

Last thing was to let the snake loose! I let it play few thousands games and the output of game was this:
